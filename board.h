...
class board {
	...
	void construct( int&& size, bool&&esc,char& r, int& i,char&& marble);
public:
	board();
	...
	string Winner()const;
	string traverse(const cell*, const int&, const int&, const int&, const int&, bool&&) const;
};
#endif /* BOARD_H */
